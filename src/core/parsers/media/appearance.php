<?php if (isset($_SESSION['user_id'])) : ?>
    <div class="w-100 pt-3 px-1">
        <button type="button" class="btn btn-sm btn-secondary shadow-none article_nav article_active float-end/" onclick="requestModal(post_modal[1], 'mediaModal', {'type':'appearance'})" <?= ((!$is_admin || !$subscription) ? 'disabled' : '') ?>> <span class=""> <i class="fas fa-media-plus me-2"></i> Upload Media Appearance</span> </button>
    </div>
<?php endif; ?>
<!-- <hr> -->
<div class="col-12">
                    ...
                    <?php if (isset($_SESSION['user_id'])) : ?>
                        ...
                        <div class="row">
                            <h5 class="col-12 font-weight-bolder p-0 px-4 pt-2">
                                <a type="button" class="text-warning" onclick="requestModal(post_modal[1], 'mediaModal', {'media_id':<?= (int) $value['media_id'] ?>})" <?= ((!$is_admin || !$subscription) ? 'disabled' : '') ?>>
                                    <?= $value['media_title'] ?>
                                </a>
                            </h5>
