...
<div class="card-body p-3">
    ...
    <form id="<?= $b_form_usr ?>" class="form-row align-items-center">
        ...
        <div class="input-group mb-3">
            ...
            <textarea class="form-control shadow-none" id="booking_message" name="booking_message" placeholder="Explain How You Led that Person" rows="4" required><?= ((isset($event['event_description'])) ? $event['event_description'] : '') ?></textarea>
        </div>

        <h5 class="mb-3 px-3 text-secondary"> You Led: </h5>

        <div class="col-12">
            <div class="input-group mb-2">
                <span class="input-group-text text-warning" style="border-right: none;"><i class="fa fa-user input_color"></i></span>
                <input type="text" class="form-control shadow-none" name="alt_name" value="<?=(isset($event['event_alt_user_name'])) ? $event['event_alt_user_name'] : '' ?>" placeholder="Name" required>
            </div>
        </div>
        <div class="col-12">
            <div class="input-group mb-2">
                <span class="input-group-text text-warning" style="border-right: none;"><i class="fa fa-user input_color"></i></span>
                <input type="text" class="form-control shadow-none" name="alt_last_name" value="<?=(isset($event['event_alt_last_name'])) ? $event['event_last_name'] : '' ?>" placeholder="Last Name" required>
            </div>
        </div>
        <div class="col-12">
            <div class="input-group mb-2">
    ...
    <form id="<?= $b_form_msg ?>" class="form-group mt-3">
        ...
        <div class="form-check form-switch mb-3">
            <input class="form-check-input" name="booking_complete" type="checkbox" role="switch" id="flexSwitchCheckChecked" <?=($event['event_processed']==1) ? 'checked' : '' ?> <?= ((!$is_admin || !$subscription) ? 'disabled' : '') ?>> <label class="form-check-label" for="flexSwitchCheckChecked">Mark as Complete</label>
        </div>
        <?php endif; ?>
    </form>
