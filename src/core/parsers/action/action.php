...
if (isset($_GET['type']) && $_GET['type'] == 'approve' && isset($_GET['uid']) && isset($_GET['token'])) {
    $registry_id = $_GET['uid'];
    ...
    if ($registry && $registry['registry_approve'] == 0) {
        ...
        if (prep_exec($sql, $dta, $sql_request_data[2])) {
            $data['success']  = true;
            $data['message']  = "You have approved the registry message";
        }
    } elseif ($registry && $registry['registry_approve'] == 1) {
        $data['error']      = true;
        $data['message']    = "You have already approved the registry message";
    } else {
        $data['error']      = true;
