...
if(isset($_POST)) {
  ...
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'task_form') {
    ...
    if (!$data['error'] && isset($chck_practice_task['task_created_date']) ) {
      $o_date           = date("Y-m-d H:i:s", strtotime($chck_practice_task['task_created_date'] . "+". $time_lapse . " min"));
      if ($o_date < $date)
      $data['error']    = true;
      $data['message']  = 'A similar task with the same name has been created in the last ' . $time_lapse . ' minutes';
    }
    ...
    if (!$data['error']) {
      ...
      if (prep_exec($sql, $dta, $sql_request_data[2])) {
        ...
        if(!$task) {
          ...
          $task_users         = (!empty($admins)) ? $admins : array();

          if ($task_checker) {
            $task_users[]       = $task_checker;
          }

          // send email to task admins
          if (!empty($task_users)) :
            foreach ($task_users as $key => $admin) {
