                        ...
                        <select id="user_type" name="user_type" value="" class="form-control shadow-none">
                            ...
                            <?php foreach ($usrt_qry as $key => $user_type) : ?>
                                ...
                                <?php if ($usr_arr != null && ($user_type['user_type_id'] == $usr_arr['user_type_id']) || $dft_user_type == $user_type['user_type']) : ?>
                                    <option value="<?= $user_type['user_type_id'] ?>" selected <?= ($usr_arr && $usr_arr['user_type_id'] == 1) ? 'disabled' : ''?>> <?= ucfirst($user_type['user_type']) ?> </option>
                                <?php else : ?>
                                    <option value="<?= $user_type['user_type_id'] ?>" <?= ($usr_arr && $usr_arr['user_type_id'] == 1) ? 'disabled' : ''?>><?= ucfirst($user_type['user_type']) ?></option>
                                <?php endif; ?>
                            <?php endforeach; ?>
                        </select>
                        <label for="event_title"> User Type </label>
                        <div id="" class="valid-feedback ps-3 mt-0">
                ...
                <div class="mb-3">
                    ...
                    <div class="ms-3 col">
                        <div class="form-check form-switch me-3">
                            <input type="checkbox" name="is_checker" class="form-check-input" value="<?= ((isset($usr_arr['user_is_checker']) ) ? $usr_arr['user_is_checker'] : 0) ?>" id="is_checker" <?= ((isset($usr_arr['user_is_checker']) && $usr_arr['user_is_checker'] == 1) ? 'checked' : '') ?>>
                            <label class="form-check-label font-weight-bolder text-secondary ms-1" for="is_checker"> Is user a checker? </label>
                        </div>
                    </div>
                </div>
        ...
        <div class="col-sm-6">
            ...
            <form id="product_form_img" class="form-horizontal" action="includes/action/create_product.php" method="POST" enctype="multipart/form-data">
                ...
                <div id="profile_img_form" class="text-center body_element/">
                    <div id="img_cspture_img" class="row shadow-md w-100 border-radius-lg p-3">
                        <p class="col-12">
    ...
    <div class="row">
        ...
        <div class="col-12 mt-3">
            <button class="btn border-radius-lg btn-dark <?= (empty($usr_arr)) ? 'col-12' : 'col-6' ?> me-3" onclick="modal_user_post()" <?= ((!$is_admin) ? 'disabled' : '') ?>> <?= (($usr_arr != null) ? 'Edit' : 'Add') ?> User</button>
            <?php if (!empty($usr_arr)): ?>
                <button type="button" class="btn btn-danger float-end" style="border-radius: 12px; font-weight: bolder" onclick="postCheck('error_pop', {'user' : parseInt(<?= (isset($usr_arr['user_id'])) ? $usr_arr['user_id'] : 0 ?>), 'form_type': 'remove_user'})" <?= ((!$is_admin || ($usr_arr['user_id'] == $_SESSION['user_id']) || !$_SESSION['is_admin'] || $usr_type['user_type_admin']) ? 'disabled' : '') ?>> Remove user </button>
            <?php endif; ?>
        </div>
    </div>
