...
<div class="row">
  <div class="col-12 mt-4">
    <?php if (isset($media_id) && !empty($media_id)) : ?>
      <button type="button" class="btn shadow-none text-danger float-end border-radiusl-lg" onclick="postCheck('error_pop', {'media_remove':true,'media_id':<?= $req_res['media_id'] ?>}, 0);" <?= ((!$is_admin) ? 'disabled' : '') ?>> <i class="fa fa-trash me-2" aria-hidden="true"></i> Remove</button>
    <?php endif; ?>
    <button type="button" class="btn btn-sm btn-secondary shadow-none border-radiusl-lg" onclick="postCheck('error_pop',  $('#media_form').serialize(), 0, true)" <?= ((!$is_admin) ? 'disabled' : '') ?>>Save changes</button>
  </div>
</div>
