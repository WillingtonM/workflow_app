<div class="container">
    <div class="row">
        ...
        <div class="col-12">
            <button class="btn btn-secondary border-radius-lg cursor-pointer" type="button" onclick="requestModal(post_modal[10], post_modal[10], {'user_type':'guest'})" <?= ((!$is_admin) ? 'disabled' : '') ?>> Add applicant | executor </button>
            ...
            <?php else : ?>
                <div class="row shadow-sm px-0 mb-3 bg-white rounded" style="border-radius: 15px !important;">
                    ...
                    if (HOST_IS_LIVE) {
                        $rgst_sql       = "SELECT * FROM users WHERE user_type = ? AND user_status = 1 ORDER BY date_created AND company_id = ? DESC LIMIT $sql_pg_strt, $intval";
