...
if(isset($_POST)) {
  ...
  if (isset($_POST['position']) && isset($_POST['email']) && isset($_POST['name'])) {
    ...
    $user_type_id       = ((isset($user['user_type_id']) && $user['user_type'] == 'sys_admin') || empty($user)) ? 1 : $user_type_id;

    $usr_type_qry       = get_user_type_by_id($user_type_id);

    if (!$usr_type_qry && ( (isset($user['user_type']) && $user_type_id != 1) || !$user) ) {
      $data['error']    = true;
      $data['message']  = 'Please select a user type option';
    }

    $required           = ($usr_type_qry && $usr_type_qry['user_type_slug'] != 'guest' && !$data['error']) ? array('username') : array('username', 'mobile');
    ...
    if ($usr_type_qry && $usr_type_qry['user_type_slug'] != 'guest' && !$data['error']) {
    ...
    if (!$data['error']) {
      ...
      if ($chck_qry && $data['data'] != 'update') {
        ...
        $user_type          = ($user_type_id == 1) ? 'sys_admin' : $usr_type_qry['user_type_slug'];
        $msg_dta            = array();
  ...
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'create_user') {
    ...
    if ($user) {
      ...
      if ($company      = get_company_by_id($company_id)) {
    ...
    if (!$data['error']) {
        ...
        if ($valid_username !== true) {
            $data['error']    = true;
            $data['message']  = $valid_username;
        }
    }

    // company handling
    $company_id           = (isset($user['company_id'])) ? $company_id : NULL;
    ...
    if (!$data['error'] && empty($event)) {
      $data['error']      = true;
      $data['message']    = 'The event enquiry is not found';
    }
    
    if (!$data['error'] && $event) {
      if (isset($user) && !empty($user)) {
        $chck_sql         = "SELECT username, user_id FROM users WHERE user_id = ? LIMIT 1";
        $chck_dta         = [$user['user_id']];
        $data['data']     = 'update';
      } else {
        $chck_sql         = "SELECT username, user_id FROM users WHERE username = ? OR email = ?LIMIT 1";
        $chck_dta         = [$username, $email];
      }
      ...
      if ($chck_qry && $data['data'] != 'update') {
        $data['error']    = true;
        $data['message']  = "The user already exists, use a different username or email";
      } else {

        // handle office & company
        if (!empty($company_id) && $user) {
          $data['data']   = 'update';

          // company
          $office         = get_office_by_company_id($company_id);
          $office_id      = (isset($office['office_id'])) ? $office['office_id'] : 0;
          ...
          // office
          $off_sql        = "UPDATE offices SET user_id = ?, office_name = ?, office_short = ?, office_slug = ?, office_telephone = ? WHERE office_id = ? AND office_status = 1 LIMIT 1";
          $off_dta        = [$user_id, $company_name, $company_short, $company_slug, $mobile, $office_id];
          ...
          if ($password != '') {
            $inst_sql     = "UPDATE users SET user_type_id = ?, user_type = ?, username = ?, contact_number = ?, name = ?, last_name = ?, email = ?, password = ? WHERE user_id = ? LIMIT 1";
            $inst_dta     = [$user_type_id, $user_type, $username, $mobile, $name, $surname, $email, $h_password, $user_id];
          } else {
            $inst_sql     = "UPDATE users SET user_type_id = ?, user_type = ?, username = ?, contact_number = ?, name = ?, last_name = ?, email = ? WHERE user_id = ? LIMIT 1";
            $inst_dta     = [$user_type_id, $user_type, $username, $mobile, $name, $surname, $email, $user_id];
          }
        } else {
          // company
          $sql_sql        = "INSERT INTO companies (user_id, company_name, company_slug, company_short, company_type, company_description, company_subscription, company_subscription_active, company_subscription_active_date) VALUES (?,?,?,?,?,?,?,?,?)";
          $dta_sql        = [$user_id, $company_name, $company_short, $company_slug, $company_type, $company_description, 1, 1, $date];

          // office
          $off_sql        = "INSERT INTO offices (company_id, user_id, office_name, office_slug, office_short, office_telephone) VALUES (?,?,?,?,?,?)";

          // user
          $h_password     = password_hashing($password);
          $inst_sql       = "INSERT INTO users (company_id, office_id, user_type_id, user_type, username, contact_number, name, last_name, email, password, date_created, last_seen, user_is_checker) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
        }

        if (prep_exec($sql_sql, $dta_sql, $sql_request_data[2])) {
          $company_id     = ($data['data'] == 'update' && !empty($company_id)) ? $company_id : $connect->lastInsertId();

          if($data['data'] != 'update') {
            $off_dta      = [$company_id, $user_id, $company_name, $company_short, $company_slug, $mobile];
          }

          if (prep_exec($off_sql, $off_dta, $sql_request_data[2])) {
            $office_id    = ($data['data'] == 'update' && !empty($office_id)) ? $office_id : $connect->lastInsertId();

            if($data['data'] != 'update') {
              $inst_dta   = [$company_id, $office_id, $user_type_id, $user_type, $username, $mobile, $name, $surname, $email, $h_password, $date, $date, 1];
            }

            if (prep_exec($inst_sql, $inst_dta, $sql_request_data[2])) {
              $usr_id     = ($data['data'] == 'update' && isset($usr_id)) ? $usr_id : $connect->lastInsertId();

              $evnt_sql   = "UPDATE events SET event_processed = 1 WHERE event_id = ? LIMIT 1";
              $evnt_dta   = [$event_id];
              prep_exec($evnt_sql, $evnt_dta, $sql_request_data[2]);


              if ($data['data'] != 'update') {
                $full_name      = $event['event_user_name'] . ' ' . $event['event_last_name'];
                $booking_email  = $event['event_user_email'];
                ...
                $client_ifo     = array(
                  ...
                  "message_notif"     => true,
                  "message_type"      => 'admin',
                  "message_text_1"    => 'We would like to kindly inform your account request has been successfully accepted.',
                  "message_text_2"    => 'Here are the login details for your new account; <br>Username: ' . $booking_email . ' or ' . $username . '<br>Password: ' . $password,
                  "message_text_3"    => 'Please change the tempary password within 3 days.',
                );
            ...
            $data['error']      = true;
            $data['message']    = 'Office was not created, something went wrong';
          }
        } else {
          $data['error']        = true;
          $data['message']      = 'Company was not created, something went wrong';
        }
  ...
  // remove event
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'event_remove') {
    $event_id       = (isset($_POST['event'])) ? $_POST['event'] : '';
    ...
    if (!$data['error'] && empty($event)) {
      $data['error']    = true;
      $data['message']  = "Event does not exists";
    }

    if (!$data['error']) {
      $sql = "UPDATE events SET event_status = 0 WHERE event_id = ? LIMIT 1";
      $dta = [$event_id];
  ...
  if (isset($_POST['form_type']) && $_POST['form_type'] == 'member_add') {
    ...
    if (!$data['error'] && !empty($practice_area)) {
      if (get_practice_area_by_id($practice_area)) {
        $data['error']      = false;
        $data['message']    = "Unknown practice area has been selected";
      }
    }
